...
const AgeFilter = ({ translation, minValue, maxValue, onChange, minLimit = 0, maxLimit = 99 }) => {
	...
	const onChangeInputHandler =
		...
		({ target: { value } }) => {
			const isMax = !isMin;
			const [min, max] = [parseInt(isMin ? value : minValue), parseInt(isMax ? value : maxValue)];
			...
			<div className="age-filter-container o-grid o-grid--form u-flex-wrap--no-wrap">
				<div className="o-grid-6">
					<Input
						id="min"
						name="min"
						handleChange={onChangeInputHandler(true, ({ min, max }) => onChange({ min, max }))}
						onBlur={onChangeInputHandler(true, ({ min, max }) => onChange({ min: getMinValue({ min, max }), max }))}
				...
				<div className="o-grid-6">
					<Input
						id="max"
						name="max"
						handleChange={onChangeInputHandler(false, ({ min, max }) => onChange({ min, max }))}
						onBlur={onChangeInputHandler(false, ({ min, max }) => onChange({ min, max: getMaxValue({ min, max }) }))}
