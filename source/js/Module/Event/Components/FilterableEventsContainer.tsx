...
class FilterableEventsContainer extends React.Component {
	...
	collectUrlParams = () => {
		...
		const stateObj = parameterValues.reduce((acc, curr) => ({ ...acc, [curr.param]: curr.value }), {});
	...
	setQueryString = () => {
		...
		tagIds.length > 0 ||
		ageRangeFilter.min > ageRangeLimit.min ||
		ageRangeFilter.max < ageRangeLimit.max ||
	...
	getTaxonomyIds = (taxonomies) => {
		...
		return taxonomies.filter((tax) => tax.checked).map((tax) => tax.id);
	};
	...
	getEvents = () => {
		...
		const { distance, groups, lang, lat, lng, moduleId, postId, restUrl, settings, translation } = this.props;
		...
		if (checkedTags.length > 0 || (checkedTags.length === 0 && settings.mod_event_tags_show === true)) {
			tags = checkedTags;
		}
		// Collect IDS
	...
	render() {
		...
		return (
			<div class="o-grid">
				...
				<div
					ref={this.myRef}
					className={`modularity-event-index__body ${!isLoaded && items.length > 0 ? 'is-disabled' : ''}`}
				>
					{!isLoaded && (
						<div
							className={`modularity-event-index__loader modularity-event-index__loader--top ${items.length > 0 ? 'modularity-event-index__loader--float' : ''}`}
						>
							<PreLoader />
						</div>
					)}
					...
					<div className="o-grid-12 modularity-event-index__content">
						{(error || (isLoaded && items.length === 0)) && <span>{translation.noEventsFound}</span>}
