...
class ApiUrl
{
    ...
    public static function buildApiUrl()
    {
        ...
        $fromDate = (is_array($occasion) && isset($occasion[0]['start_date']) && strtotime($occasion[0]['start_date']) < strtotime('now'))
            ? date('Y-m-d', strtotime($occasion[0]['start_date']))
            : date('Y-m-d');
        ...
        $distance = (get_field('event_geographic_distance', 'option') && $importLocation == 'point')
            ? '&distance=' . get_field('event_geographic_distance', 'option')
            : '';
        ...
        if (is_array($selectedGroups) && !empty($selectedGroups)) {
            $groupValues = array_column($selectedGroups, 'slug');
            $groups = '&group-id=' . implode(',', array_filter($groupValues));
        }
